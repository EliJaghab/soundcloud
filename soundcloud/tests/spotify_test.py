...
def test_get_track_id_lucid():
    s = Spotify()
    data = s.get_track_id("Lucid Dreams", "Juice WRLD")
    expected = "285pBltuF7vW8TeWk8hdRR"
    assert data[0] == expected

def test_get_track_id_roses():
    s = Spotify()
    data = s.get_track_id("Roses", "benny")
...
def test_get_track_id_memories():
    s = Spotify()
    data = s.get_track_id("Memories (feat. Kid Cudi) [2021 Remix] (2021 Remix)", "David Guetta")
    expected = "59o6ojGNGJOYiVJSzC6Lsa"
    assert data[0] == expected

def test_get_track_id_split_example():
    s = Spotify()
    data = s.get_track_id("Drake - A Keeper", "octobersveryown")
    expected = "0nAZGkBGKQCXyaoSJfRhC1"
    assert data[0] == expected

def test_get_track_id_giving_up():
    s = Spotify()
    data = s.get_track_id("Giving Up (feat. Mafro)", "TSHA")
